...
render (GameState _ _ score _ _ _ Lose)
  = pictures (map renderEnt (controls:score':[loser]))
    where (Score s, _) = score
          loser    = (Loser,((-120/2,-20/2),(0,0)))
          score'   = (Score s,((-120/2,-40/2),(0,0)))
          controls = (Controls,((-630/2,385/2),(0,0)))
render (GameState _ _ score _ _ _ Win)
  = pictures (map renderEnt (controls:score':[winner]))
    where (Score s, _) = score
          winner = (Winner,((-120/2,-20/2),(0,0)))
          score' = (Score s,((-120/2,-40/2),(0,0)))
          controls = (Controls,((-630/2,385/2),(0,0)))
render (GameState bar ball score level lives blocks _)
  = pictures (map renderEnt (controls:bar:ball:score:level:lives:(blocksToList blocks)))
    where controls = (Controls,((-630/2,385/2),(0,0)))
...
renderShape (Score s)  = color white (scale (0.15/2) (0.15/2) (text txt))
  where txt = "Score: " ++ show(s)
renderShape (Level l)  = color white (scale (0.15/2) (0.15/2) (text txt))
  where txt = "Level: " ++ show(l)
renderShape (Lives l)  = color white (scale (0.15/2) (0.15/2) (text txt))
  where txt = "Lives: " ++ show(l)
renderShape (Loser)    = color white (scale (0.5/2) (0.5/2) (text txt))
  where txt = "LOSER!"
renderShape (Winner)   = color white (scale (0.5/2) (0.5/2) (text txt))
  where txt = "WINNER!"
renderShape (Controls) = color white (scale (0.1/2) (0.1/2) (text txt))
